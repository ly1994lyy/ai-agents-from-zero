...
所以可以先记住一句最重要的话：
**`@tool` 的意义，不是把函数“变复杂”，而是把函数“变成模型看得懂的工具”。**
...
**Pydantic** 是 Python 里非常常用的数据校验库，本质上就是把“参数结构、参数类型、字段说明、校验规则”统一收敛到一个模型类里。\*\*
...
- 和 [第 20 章MCP模型上下文协议](20-MCP模型上下文协议.md) 的关系：MCP 解决的是“如何用标准协议把外部能力开放给模型”，可以理解成更通用、更标准化的工具接入方式

如果用一句话总结它们的关系：

**Tool 提供能力，Memory 提供上下文，Agent 负责决策，MCP 负责标准化接入。**

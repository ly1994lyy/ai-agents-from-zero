...
- Docker Compose 说明：https://docs.docker.com/compose/
- Dify Self-hosted：https://docs.dify.ai/zh-hans/getting-started/install-self-hosted/docker-compose
...
1. 镜像、容器、数据卷三者在 Docker 体系里的关系是什么？
   ...
   **答案：** 镜像是应用模板，容器是镜像运行后的实例，数据卷是容器外部的持久化存储。可以把它理解成：镜像像安装包，容器像正在运行的程序，数据卷像独立保存数据的硬盘目录。
